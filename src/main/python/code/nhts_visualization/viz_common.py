...
MODE_LABELS = {
    1: "Car", 2: "Van", 3: "SUV", 4: "Pickup", 5: "Other truck",
    6: "RV", 7: "Motorcycle", 8: "Golf cart", 9: "Local bus",
    10: "Commuter bus", 11: "School bus", 12: "Charter bus",
    13: "Intercity bus", 14: "Shuttle bus", 15: "Amtrak/intercity rail",
    16: "Commuter rail", 17: "Subway/elevated", 18: "Streetcar/trolley",
    19: "Taxi", 20: "Ferry", 21: "Airplane", 22: "Bicycle",
    23: "Walk", 24: "Paratransit", 97: "Other",
}
MODE_GROUPS = {
    1: "Private vehicle", 2: "Private vehicle", 3: "Private vehicle", 4: "Private vehicle",
    5: "Private vehicle", 6: "Private vehicle", 7: "Private vehicle", 8: "Private vehicle",
    9: "Transit", 10: "Transit", 11: "Bus", 12: "Bus", 13: "Bus", 14: "Bus",
    15: "Rail", 16: "Rail", 17: "Rail", 18: "Rail", 19: "Taxi",
    20: "Ferry", 21: "Air", 22: "Bicycle", 23: "Walk", 24: "Paratransit", 97: "Other",
}
PURPOSE_SUMMARY = {
    1: "Home", 10: "Work", 20: "School/religious", 30: "Medical/dental",
    40: "Shopping/errands", 50: "Social/recreational", 60: "Family/personal business",
    70: "Transport someone", 80: "Meals", 97: "Other",
}
PURPOSE_1990 = {
    1: "To/from work", 2: "Work business", 3: "Shopping",
    4: "Family/personal business", 5: "School/church", 6: "Medical/dental",
    7: "Vacation", 8: "Visit friends/relatives", 10: "Other social/recreation",
    11: "Other", 98: "N/A", 99: "Refused",
}
VEHICLE_TYPE = {
    1: "Car/station wagon", 2: "Van", 3: "SUV", 4: "Pickup truck",
    5: "Other truck", 6: "RV", 7: "Motorcycle", 8: "Golf cart", 97: "Other",
}
FUEL_TYPE = {1: "Diesel", 2: "Natural gas", 3: "Electricity", 4: "Gasoline"}
INCOME_LABELS = {
    1: "< $5k", 2: "$5-9k", 3: "$10-14k", 4: "$15-19k", 5: "$20-24k",
    6: "$25-29k", 7: "$30-34k", 8: "$35-39k", 9: "$40-44k", 10: "$45-49k",
    11: "$50-54k", 12: "$55-59k", 13: "$60-64k", 14: "$65-69k", 15: "$70-74k",
    16: "$75-79k", 17: "$80-99k", 18: "$100k+",
}
LIFECYCLE_LABELS = {
    1: "1 adult, no children", 2: "2+ adults, no children",
    3: "1 adult, youngest 0-5", 4: "2+ adults, youngest 0-5",
    5: "1 adult, youngest 6-15", 6: "2+ adults, youngest 6-15",
    7: "1 adult, youngest 16-21", 8: "2+ adults, youngest 16-21",
    9: "1 retired adult, no children", 10: "2+ retired adults, no children",
}
STATE_FIPS = {
...
def code_to_label(s: pd.Series, mapping: Mapping[int, str], unknown: str = "Unknown") -> pd.Series:
    n = numeric(s)
    return n.map(mapping).fillna(unknown)


def purpose_group(s: pd.Series) -> pd.Series:
    n = numeric(s)
    tens = np.floor(n / 10) * 10
    tens = pd.Series(tens, index=s.index)
    tens = tens.where(n != 1, 1)
    tens = tens.where(n != 97, 97)
    return tens.map(PURPOSE_SUMMARY).fillna("Unknown")


def weighted_counts(category: pd.Series, weight: pd.Series, drop_unknown: bool = False) -> pd.Series:
    d = pd.DataFrame({"category": category.astype("string"), "weight": positive_weight(weight)})
    d = d.dropna()
    if drop_unknown:
...
def barh(series: pd.Series, title: str, xlabel: str, out: Path, filename: str, top: int | None = None, percent: bool = False) -> None:
    s = series.dropna()
    if top is not None:
        s = s.head(top)
...
def heatmap(table: pd.DataFrame, title: str, out: Path, filename: str, xlabel: str = "", ylabel: str = "", fmt: str = ".1f", annotate: bool = True) -> None:
    if table.empty:
        return
...
def age_group(s: pd.Series) -> pd.Series:
    ...
    labels = ["0-4", "5-15", "16-24", "25-34", "35-44", "45-54", "55-64", "65-74", "75-89", "89+"]
...
def write_summary(out: Path, dataset: str, rows: int, figures: Iterable[str], notes: Iterable[str] = ()) -> None:
    lines = [f"Dataset: {dataset}", f"Rows read: {rows:,}", "", "Figures:"]
